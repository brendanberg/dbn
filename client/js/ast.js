...
AST.Statement.prototype.indent = function (depth) {
    ...
    return indent + this.canonical + ' ' + this.args.map((a) => a.indent()).join(' ') + comment;
};
...
AST.Program.prototype.emit = function (ctx) {
    ...
    return [Op.STACK_ALLOC, this.meta.locals.length]
        ...
        .concat([Op.STACK_FREE, this.meta.locals.length, Op.HALT])
        .concat(this.definitions.flatMap((d) => d.emit()))
        .concat(
            this.exports.flatMap((fn) => {
                ...
                return (
                    [Op.LABEL, fn.name]
                        ...
                        .concat(
                            range(fn.length)
                                .reverse()
                                .flatMap((a) => [Op.SET_ARGUMENT, a])
                        )
                        .concat([Op.INVOKE, fn.length, fn.name, Op.RETURN])
                );
            })
        )
        .concat(emitBuiltins());
...
AST.Loop.prototype.emit = function () {
    ...
    return [Op.LOCATION_PUSH, start, end]
        .concat(this.stop.emit())
        .concat([Op.DUPLICATE])
        .concat(this.start.emit())
        .concat([Op.DUPLICATE, store_iterator, this.iterator.canonical, Op.SUBTRACT, Op.DUPLICATE, Op.LABEL, _body])
        .concat(this.statements.flatMap((s) => s.emit()))
        .concat([
            Op.JUMP_IF_ZERO,
            _break,
        ...
        .concat(this.meta.inner ? [Op.REDRAW] : [])
        .concat([Op.JUMP, _body, Op.LABEL, _break, Op.POP, Op.POP, Op.LOCATION_POP]);
};
...
AST.Condition.prototype.emit = function () {
    ...
    if (this.predicate.meta.type === 'not') {
        ...
        return [Op.LOCATION_PUSH, start, end]
            ...
            .concat([bodylabel, Op.JUMP, endlabel, Op.LABEL, bodylabel])
            .concat(this.statements.flatMap((s) => s.emit()))
...
AST.LessThan.prototype.emit = function () {
    ...
    return this.lhs.emit().concat(this.rhs.emit()).concat([Op.SUBTRACT, Op.JUMP_IF_NONNEGATIVE]);
};
...
AST.Equals.prototype.emit = function () {
    ...
    return this.lhs.emit().concat(this.rhs.emit()).concat([Op.SUBTRACT, Op.JUMP_IF_NONZERO]);
};
...
AST.Command.prototype.emit = function () {
    ...
    return [
        ...
        this.name.canonical,
        Op.STACK_ALLOC,
        this.meta.locals.length + this.meta.unbound.length,
    ]
    ...
    switch (this.canonical) {
        case 'Paper': {
            if (this.args.length === 1) {
                return [Op.LOCATION_PUSH, start, end, Op.REDRAW, Op.CONSTANT, 255]
                ...
                return [Op.LOCATION_PUSH, start, end, Op.REDRAW]
                    .concat(
                        this.args.flatMap((x) => {
                            return x
                                .emit()
                    ...
                    .concat([Op.PACK_RGB, Op.FILL_CANVAS, Op.LOCATION_POP]);
            }
        }
        case 'Pen': {
            if (this.args.length === 1) {
                ...
                return [Op.LOCATION_PUSH, start, end, Op.CONSTANT, 255]
                    .concat(rvalue)
                    .concat([
                        Op.CONSTANT,
                        0,
                ...
                return [Op.LOCATION_PUSH, start, end]
                    ...
                    .concat([Op.PACK_RGB, Op.SET_PEN_COLOR, Op.LOCATION_POP]);
            }
        }
        case 'Set': {
            if (this.args[0].meta.type === 'vector') {
                ...
                if (this.args[1].meta.type === 'vector') {
                    return [Op.LOCATION_PUSH, start, end]
                        .concat(lvalue)
                        .concat(rvalue)
                        .concat([Op.PACK_GRAY, Op.WRITE_PIXEL, Op.LOCATION_POP]);
                } else {
                    return [Op.LOCATION_PUSH, start, end]
                        .concat(lvalue)
                        .concat([Op.CONSTANT, 255])
                        .concat(rvalue)
                        .concat([
                            Op.CONSTANT,
                            0,
                ...
                if (this.args[0].meta.argument === true) {
                    return [Op.LOCATION_PUSH, start, end]
                        .concat(rvalue)
                        .concat([Op.SET_ARGUMENT, this.args[0].canonical, Op.LOCATION_POP]);
                } else {
                    return [Op.LOCATION_PUSH, start, end]
                        .concat(rvalue)
                        .concat([Op.SET_LOCAL, this.args[0].canonical, Op.LOCATION_POP]);
                }
            }
        }
        case 'Line': {
            return [Op.LOCATION_PUSH, start, end]
                ...
                .concat([Op.STACK_ALLOC, 4, Op.CALL, 'line', Op.STACK_FREE, 4, Op.POP, Op.LOCATION_POP]);
        }
        case 'Field': {
            return [Op.LOCATION_PUSH, start, end]
                .concat(this.args.flatMap((a) => a.emit()))
                .concat([Op.STACK_ALLOC, 5, Op.CALL, 'field', Op.STACK_FREE, 5, Op.POP, Op.LOCATION_POP]);
        }
        case 'Value': {
            // TODO: This is essentially a return statement but it kind of
            // has to go at the end of the function definition.
            return [Op.LOCATION_PUSH, start, end].concat(this.args[0].emit()).concat([Op.LOCATION_POP]);
            /*
            .concat([
                Op.STACK_FREE, 0,
                Op.RETURN
        ...
        case 'Pause': {
            return [Op.LOCATION_PUSH, start, end].concat(this.args[0].emit()).concat(Op.PAUSE, Op.LOCATION_POP);
        }
        case 'Refresh': {
...
AST.Generator.prototype.emit = function (ctx) {
    ...
    return [Op.LOCATION_PUSH, start, end]
        .concat(this.meta.unbound.flatMap((id) => [Op.GET_LOCAL, id]))
        .concat(this.args.flatMap((a) => a.emit()))
        .concat([Op.STACK_ALLOC, this.args.length, Op.CALL, this.canonical, Op.STACK_FREE, this.args.length])
        .concat(
            this.meta.unbound
                .slice(0)
                .reverse()
